...
// ajouter au build path le tools.jar du tp1

public class MaDocDeClasseOmar extends Doclet {

	// PAS FINI IL FAUT BOUCLER SIMPLEMENT SUR LA CLASSE ET SUR SES ANNOTATIONS ....
	...
	public static boolean start(RootDoc root) { 
		...
		for (int i = 0; i < classes.length; ++i) {
	    	System.out.println();	    	
	    	if (classes[i].isClass()) System.out.print("*** classe ");
	        ...
	        for (int j = 0; j < champs.length; ++j) {
	        	System.out.print("     ");
	        	if ( champs[j].isStatic() ) System.out.print("*");
	        ...
	        for (int j = 0; j < methodes.length; ++j) {
		        	System.out.print("     ");
		        	if ( methodes[j].isStatic() ) System.out.print("*");
		        	System.out.print(afficheVisibilite(methodes[j])+" "+methodes[j].returnType()+" "+methodes[j].name());
	...
	public static void afficheParametres(ExecutableMemberDoc element) {
		...
		for(int k=0; k<argv.length; k++) {
			p=argv[k];
			if (k==0) System.out.print(p.type()+" "+p.name());
			else System.out.print(","+p.type()+" "+p.name());
		}
		System.out.print(")");		
	}
